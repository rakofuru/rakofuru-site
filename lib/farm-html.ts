...
export function buildFarmViewFromHtml(farm: Farm): FarmView {
  const view: FarmView = {
    id: farm.id,
    slug: farm.slug,
    ...
    heroImage: farm.heroImage
      ? {
          id: farm.heroImage.id,
          srcUrl: farm.heroImage.srcUrl,
          alt: farm.heroImage.alt ?? null,
          title: farm.heroImage.title ?? null,
          width: farm.heroImage.width ?? 800,
          height: farm.heroImage.height ?? 600,
        }
      : {
          id: farm.id,
    ...
    location: {
      googleMapsPlaceUrl: farm.location.googleMapsPlaceUrl ?? null,
      googleMapsReviewsUrl: farm.location.googleMapsReviewsUrl ?? null,
      addressText: farm.location.addressText ?? null,
      phone: farm.location.phone ?? null,
      officialUrl: farm.location.officialUrl ?? null,
    },
    publishedAt: farm.publishedAt,
    updatedAt: farm.updatedAt ?? farm.publishedAt,
    infoTable: {},
    pricingBrief: "",
    parkingBrief: "",
    hoursBrief: "",
  ...
  const sections: FarmView["sections"] = []
  $("h2").each((_, h2) => {
    const $h2 = $(h2)
    ...
    sections.push({ title, contentHtml: contentHtml.trim() })
  })
